...
const SymptomChecker = () => {
  ...
  useEffect(() => {
    const fetchSymptoms = async () => {
      try {
        const response = await axios.get("/api/prediction/symptoms");
        setAvailableSymptoms(response.data.symptoms);
        // Initialize symptoms with 0
  ...
  const handlePredict = async (e) => {
    ...
    try {
      const response = await axios.post(
        "/api/prediction/predict-disease",
        {
          age: parseInt(age),
          gender,
          symptoms,
      ...
      if (response.data.prediction.recommended_department) {
        const doctorResponse = await axios.get(
          `/api/prediction/doctors/${response.data.prediction.recommended_department}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
